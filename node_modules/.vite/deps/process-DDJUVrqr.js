//#region node_modules/@splinetool/runtime/build/process.js
var Module = (() => {
	var _scriptDir = typeof document !== "undefined" && document.currentScript ? document.currentScript.src : void 0;
	return (function(moduleArg = {}) {
		var Module$1 = moduleArg;
		var readyPromiseResolve, readyPromiseReject;
		Module$1["ready"] = new Promise((resolve, reject) => {
		...
		var err = Module$1["printErr"] || console.error.bind(console);
		...
		var wasmBinaryFile = "process.wasm";
		...
		var __embind_finalize_value_array = (rawTupleType) => {
			...
			whenDependentTypesAreResolved([rawTupleType], elementTypes, function(elementTypes$1) {
				elements.forEach((elt, i) => {
					var getterReturnType = elementTypes$1[i];
					var getter = elt.getter;
					var getterContext = elt.getterContext;
					var setterArgumentType = elementTypes$1[i + elementsLength];
		...
		var __embind_finalize_value_object = (structType) => {
			...
			whenDependentTypesAreResolved([structType], fieldTypes, (fieldTypes$1) => {
				...
				return [{
					...
					"toWireType": (destructors, o) => {
						for (var fieldName in fields) if (!(fieldName in o)) throw new TypeError(`Missing field: "${fieldName}"`);
						var ptr = rawConstructor();
		...
		var exposePublicSymbol = (name, value, numArguments) => {
			if (Module$1.hasOwnProperty(name)) {
				...
				if (Module$1.hasOwnProperty(numArguments)) throwBindingError(`Cannot register multiple overloads of a function with the same number of arguments (${numArguments})!`);
				Module$1[name].overloadTable[numArguments] = value;
			} else {
				Module$1[name] = value;
				if (void 0 !== numArguments) Module$1[name].numArguments = numArguments;
			}
		};
		...
		var replacePublicSymbol = (name, value, numArguments) => {
			if (!Module$1.hasOwnProperty(name)) throwInternalError("Replacing nonexistant public symbol");
		...
		var __embind_register_class = (rawType, rawPointerType, rawConstPointerType, baseClassRawType, getActualTypeSignature, getActualType, upcastSignature, upcast, downcastSignature, downcast, name, destructorSignature, rawDestructor) => {
			...
			whenDependentTypesAreResolved([
				...
				if (registeredClass.baseClass) {
					if (registeredClass.baseClass.__derivedClasses === void 0) registeredClass.baseClass.__derivedClasses = [];
		...
		function craftInvokerFunction(humanName, argTypes, classType, cppInvokerFunc, cppTargetFunc, isAsync) {
			...
			for (var i = 1; i < argTypes.length; ++i) if (argTypes[i] !== null && argTypes[i].destructorFunction === void 0) {
		...
		var __embind_register_class_property = (classType, fieldName, getterReturnType, getterSignature, getter, getterContext, setterArgumentType, setterSignature, setter, setterContext) => {
			...
			whenDependentTypesAreResolved([], [classType], function(classType$1) {
				classType$1 = classType$1[0];
				var humanName = `${classType$1.name}.${fieldName}`;
				var desc = {
					get() {
				...
				if (setter) desc.set = () => throwUnboundTypeError(`Cannot access ${humanName} due to unbound types`, [getterReturnType, setterArgumentType]);
				else desc.set = (v) => throwBindingError(humanName + " is a read-only property");
				Object.defineProperty(classType$1.registeredClass.instancePrototype, fieldName, desc);
				whenDependentTypesAreResolved([], setter ? [getterReturnType, setterArgumentType] : [getterReturnType], function(types) {
					var getterReturnType$1 = types[0];
					var desc$1 = {
						get() {
							var ptr = validateThis(this, classType$1, humanName + " getter");
							return getterReturnType$1["fromWireType"](getter(getterContext, ptr));
						},
						enumerable: true
					};
					if (setter) {
						setter = embind__requireFunction(setterSignature, setter);
						var setterArgumentType$1 = types[1];
						desc$1.set = function(v) {
							var ptr = validateThis(this, classType$1, humanName + " setter");
							var destructors = [];
							setter(setterContext, ptr, setterArgumentType$1["toWireType"](destructors, v));
							runDestructors(destructors);
						};
					}
					Object.defineProperty(classType$1.registeredClass.instancePrototype, fieldName, desc$1);
					return [];
				});
				return [];
			});
		...
		var count_emval_handles = () => {
			var count = 0;
			for (var i = emval_handles.reserved; i < emval_handles.allocated.length; ++i) if (emval_handles.allocated[i] !== void 0) ++count;
			return count;
		};
		var init_emval = () => {
		...
		var Emval = {
			...
			toHandle: (value) => {
				switch (value) {
					...
					case true: return 3;
					case false: return 4;
					default: return emval_handles.allocate({
						refcount: 1,
						value
					});
				}
		...
		var __embind_register_function = (name, argCount, rawArgTypesAddr, signature, rawInvoker, fn, isAsync) => {
			...
			whenDependentTypesAreResolved([], argTypes, function(argTypes$1) {
		...
		var __embind_register_std_string = (rawType, name) => {
			...
			registerType(rawType, {
				...
				"toWireType"(destructors, value) {
					...
					else if (valueIsOfTypeString) for (var i = 0; i < length; ++i) {
		...
		var FS = {
			...
			createStandardStreams() {
				if (Module$1["stdin"]) FS.createDevice("/dev", "stdin", Module$1["stdin"]);
				else FS.symlink("/dev/tty", "/dev/stdin");
				if (Module$1["stdout"]) FS.createDevice("/dev", "stdout", null, Module$1["stdout"]);
				else FS.symlink("/dev/tty", "/dev/stdout");
				if (Module$1["stderr"]) FS.createDevice("/dev", "stderr", null, Module$1["stderr"]);
			...
			ensureErrnoError() {
				...
				FS.ErrnoError = function ErrnoError(errno, node) {
					...
					this.node = node;
					this.setErrno = function(errno$1) {
						this.errno = errno$1;
					};
					this.setErrno(errno);
					this.message = "FS error";
				};
				FS.ErrnoError.prototype = /* @__PURE__ */ new Error();
		...
		var _strftime = (s, maxsize, format, tm) => {
			...
			function getWeekBasedYear(date$1) {
				var thisDate = addDays(new Date(date$1.tm_year + 1900, 0, 1), date$1.tm_yday);
			...
			var EXPANSION_RULES_2 = {
				"%a": (date$1) => WEEKDAYS[date$1.tm_wday].substring(0, 3),
				"%A": (date$1) => WEEKDAYS[date$1.tm_wday],
				"%b": (date$1) => MONTHS[date$1.tm_mon].substring(0, 3),
				"%B": (date$1) => MONTHS[date$1.tm_mon],
				"%C": (date$1) => {
					return leadingNulls((date$1.tm_year + 1900) / 100 | 0, 2);
				},
				"%d": (date$1) => leadingNulls(date$1.tm_mday, 2),
				"%e": (date$1) => leadingSomething(date$1.tm_mday, 2, " "),
				"%g": (date$1) => getWeekBasedYear(date$1).toString().substring(2),
				"%G": (date$1) => getWeekBasedYear(date$1),
				"%H": (date$1) => leadingNulls(date$1.tm_hour, 2),
				"%I": (date$1) => {
				...
				"%j": (date$1) => leadingNulls(date$1.tm_mday + arraySum(isLeapYear(date$1.tm_year + 1900) ? MONTH_DAYS_LEAP : MONTH_DAYS_REGULAR, date$1.tm_mon - 1), 3),
				"%m": (date$1) => leadingNulls(date$1.tm_mon + 1, 2),
				"%M": (date$1) => leadingNulls(date$1.tm_min, 2),
				"%n": () => "\n",
				"%p": (date$1) => {
					if (date$1.tm_hour >= 0 && date$1.tm_hour < 12) return "AM";
					return "PM";
				},
				"%S": (date$1) => leadingNulls(date$1.tm_sec, 2),
				"%t": () => "	",
				"%u": (date$1) => date$1.tm_wday || 7,
				"%U": (date$1) => {
					var days = date$1.tm_yday + 7 - date$1.tm_wday;
					return leadingNulls(Math.floor(days / 7), 2);
				},
				"%V": (date$1) => {
					var val = Math.floor((date$1.tm_yday + 7 - (date$1.tm_wday + 6) % 7) / 7);
					if ((date$1.tm_wday + 371 - date$1.tm_yday - 2) % 7 <= 2) val++;
					if (!val) {
						val = 52;
						var dec31 = (date$1.tm_wday + 7 - date$1.tm_yday - 1) % 7;
						if (dec31 == 4 || dec31 == 5 && isLeapYear(date$1.tm_year % 400 - 1)) val++;
					} else if (val == 53) {
						var jan1 = (date$1.tm_wday + 371 - date$1.tm_yday) % 7;
						if (jan1 != 4 && (jan1 != 3 || !isLeapYear(date$1.tm_year))) val = 1;
					}
					return leadingNulls(val, 2);
				},
				"%w": (date$1) => date$1.tm_wday,
				"%W": (date$1) => {
					var days = date$1.tm_yday + 7 - (date$1.tm_wday + 6) % 7;
					return leadingNulls(Math.floor(days / 7), 2);
				},
				"%y": (date$1) => (date$1.tm_year + 1900).toString().substring(2),
				"%Y": (date$1) => date$1.tm_year + 1900,
				"%z": (date$1) => {
		...
		InternalError = Module$1["InternalError"] = class InternalError$1 extends Error {
			constructor(message) {
				super(message);
		...
		BindingError = Module$1["BindingError"] = class BindingError$1 extends Error {
			constructor(message) {
				super(message);
		...
		UnboundTypeError = Module$1["UnboundTypeError"] = extendError(Error, "UnboundTypeError");
		handleAllocatorInit();
		init_emval();
		...
		var _malloc = Module$1["_malloc"] = (a0) => (_malloc = Module$1["_malloc"] = wasmExports["M"])(a0);
		var _free = Module$1["_free"] = (a0) => (_free = Module$1["_free"] = wasmExports["N"])(a0);
		var ___getTypeName = (a0) => (___getTypeName = wasmExports["P"])(a0);
		var __embind_initialize_bindings = Module$1["__embind_initialize_bindings"] = () => (__embind_initialize_bindings = Module$1["__embind_initialize_bindings"] = wasmExports["Q"])();
		...
		var dynCall_jiji = Module$1["dynCall_jiji"] = (a0, a1, a2, a3, a4) => (dynCall_jiji = Module$1["dynCall_jiji"] = wasmExports["V"])(a0, a1, a2, a3, a4);
		var dynCall_viijii = Module$1["dynCall_viijii"] = (a0, a1, a2, a3, a4, a5, a6) => (dynCall_viijii = Module$1["dynCall_viijii"] = wasmExports["W"])(a0, a1, a2, a3, a4, a5, a6);
		var dynCall_iiiiij = Module$1["dynCall_iiiiij"] = (a0, a1, a2, a3, a4, a5, a6) => (dynCall_iiiiij = Module$1["dynCall_iiiiij"] = wasmExports["X"])(a0, a1, a2, a3, a4, a5, a6);
		var dynCall_iiiiijj = Module$1["dynCall_iiiiijj"] = (a0, a1, a2, a3, a4, a5, a6, a7, a8) => (dynCall_iiiiijj = Module$1["dynCall_iiiiijj"] = wasmExports["Y"])(a0, a1, a2, a3, a4, a5, a6, a7, a8);
		var dynCall_iiiiiijj = Module$1["dynCall_iiiiiijj"] = (a0, a1, a2, a3, a4, a5, a6, a7, a8, a9) => (dynCall_iiiiiijj = Module$1["dynCall_iiiiiijj"] = wasmExports["Z"])(a0, a1, a2, a3, a4, a5, a6, a7, a8, a9);
		Module$1["ccall"] = ccall;
		var calledRun;
...
//# sourceMappingURL=process-DDJUVrqr.js.map